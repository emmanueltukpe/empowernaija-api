...
import {
  CreateCorporateDonationDto,
  UpdateCorporateDonationDto,
} from "../dto/corporate-donation.dto";
...
export class DonationController extends BaseController {
  ...
  async recordDonation(
    @Body() dto: CreateCorporateDonationDto
  ...
  async getBusinessDonations(
    ...
    return await this.donationService.getBusinessDonations(
      businessId,
  ...
  @ApiOperation({
    ...
    description:
      "Calculate 10% deduction for all approved donations in a tax year",
  })
  @ApiParam({ name: "businessId", description: "Business ID" })
  ...
  async calculateDeduction(
    ...
    return await this.donationService.calculate10PercentDeduction(
      businessId,
      Number(taxYear)
    );
  }
